...
let filter_typing_context_by_var (typing_context : typing_context)
    (var : Var_name.t) : typing_context =
  List.filter typing_context ~f:(fun (TypingContextEntry (var_entry, _)) ->
      Var_name.( = ) var var_entry)

let check_typing_context_shadow (typing_context : typing_context)
...
let extend_typing_context (typing_context : typing_context) (var : Var_name.t)
    (var_type : type_expr) : typing_context Or_error.t =
  let open Result in
