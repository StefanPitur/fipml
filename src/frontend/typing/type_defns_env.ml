...
let assert_custom_type_in_types_env
  (loc : loc)
  ...
  =
  let open Result in
  if List.mem types_env type_name ~equal:Type_name.( = )
  then Ok ()
  else (
    let error_string =
      Fmt.str
        "%s. Type %s not defined"
        (string_of_loc loc)
...
let assert_custom_type_not_in_types_env
  (loc : loc)
  (type_name : Type_name.t)
  (types_env : types_env)
  : unit Or_error.t
  =
  let open Result in
  if not (List.mem types_env type_name ~equal:Type_name.( = ))
  then Ok ()
  else (
    let error_string =
      Fmt.str
        "%s. Duplicate definition of type %s"
        (string_of_loc loc)
        (Type_name.to_string type_name)
    in
...
let filter_constructors_env_by_name
  (constructor_name : Constructor_name.t)
  (constructors_env : constructors_env)
  : constructors_env
  =
  List.filter constructors_env ~f:(fun (ConstructorEnvEntry (_, constructor, _)) ->
    Constructor_name.( = ) constructor_name constructor)
;;

let assert_constructor_in_constructors_env
  (loc : loc)
  (constructor_name : Constructor_name.t)
  (constructors_env : constructors_env)
  : unit Or_error.t
...
let assert_constructor_not_in_constructors_env
  (loc : loc)
  (constructor_name : Constructor_name.t)
  (constructors_env : constructors_env)
  : unit Or_error.t
  ...
  | _ ->
    let error_string =
      Fmt.str
        "%s. Duplicate definition of constructor %s"
        (string_of_loc loc)
        (Constructor_name.to_string constructor_name)
    in
