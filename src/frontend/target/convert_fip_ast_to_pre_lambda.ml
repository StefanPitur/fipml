...
let rec convert_fip_ast_to_pre_lambda
  ...
  | Let (_, _, _, _, vars, vars_expr, expr) ->
      let pre_lambda_vars_expr =
        convert_fip_ast_to_pre_lambda constructor_tag_map vars_expr
      in
      let pre_lambda_expr =
        convert_fip_ast_to_pre_lambda constructor_tag_map expr
      in
  ...
  | If (_, _, _, _, expr_cond, expr_then) ->
      let pre_lambda_expr_cond =
        convert_fip_ast_to_pre_lambda constructor_tag_map expr_cond
      in
      let pre_lambda_expr_then =
        convert_fip_ast_to_pre_lambda constructor_tag_map expr_then
      in
  ...
  | IfElse (_, _, _, _, expr_cond, expr_then, expr_else) ->
      ...
      let pre_lambda_expr_else =
        convert_fip_ast_to_pre_lambda constructor_tag_map expr_else
      in
  ...
  | Match (_, _, _, _, matched_var, patterns) ->
      let pre_lambda_patterns =
        List.map patterns
          ~f:(convert_fip_ast_to_pre_lambda_pattern constructor_tag_map)
      in
      Match (NonDestructive, matched_var, pre_lambda_patterns)
  | DMatch (_, _, _, _, matched_var, patterns) ->
      let pre_lambda_patterns =
        List.map patterns
          ~f:(convert_fip_ast_to_pre_lambda_pattern constructor_tag_map)
      in
      Match (Destructive, matched_var, pre_lambda_patterns)
  ...
  | BinaryOp (_, _, _, _, binary_op, expr_left, expr_right) ->
      let pre_lambda_expr_left =
        convert_fip_ast_to_pre_lambda constructor_tag_map expr_left
      in
      let pre_lambda_expr_right =
        convert_fip_ast_to_pre_lambda constructor_tag_map expr_right
      in
...
and convert_fip_ast_to_pre_lambda_pattern
  ...
  let pre_lambda_matched_expr =
    convert_fip_ast_to_pre_lambda_matched_expr matched_expr
  in
  let pre_lambda_expr =
    convert_fip_ast_to_pre_lambda constructor_tag_map expr
  in
  MPattern (pre_lambda_matched_expr, pre_lambda_expr)
  ...
  | MConstructor (_, _, constructor_name, constructor_matched_exprs) ->
      let pre_lambda_constructor_matched_exprs =
        List.map constructor_matched_exprs
          ~f:convert_fip_ast_to_pre_lambda_matched_expr
      in
...
let convert_fip_ast_to_pre_lambda_function_defn
  ...
  let pre_lambda_function_body =
    convert_fip_ast_to_pre_lambda constructor_tag_map function_body
  in
...
let convert_fip_ast_to_pre_lambda_function_defns
  ...
  List.map fip_ast_function_defns
    ~f:(convert_fip_ast_to_pre_lambda_function_defn constructor_tag_map)
