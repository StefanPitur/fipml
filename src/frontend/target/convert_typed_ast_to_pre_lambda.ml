...
let rec convert_typed_ast_to_pre_lambda
  ...
  | Let (_, _, _, vars, vars_expr, expr) ->
      let pre_lambda_vars_expr =
        convert_typed_ast_to_pre_lambda constructor_tag_map vars_expr
      in
      let pre_lambda_expr =
        convert_typed_ast_to_pre_lambda constructor_tag_map expr
      in
  ...
  | If (_, _, expr_cond, expr_then) ->
      let pre_lambda_expr_cond =
        convert_typed_ast_to_pre_lambda constructor_tag_map expr_cond
      in
      let pre_lambda_expr_then =
        convert_typed_ast_to_pre_lambda constructor_tag_map expr_then
      in
  ...
  | IfElse (_, _, expr_cond, expr_then, expr_else) ->
      ...
      let pre_lambda_expr_else =
        convert_typed_ast_to_pre_lambda constructor_tag_map expr_else
      in
  ...
  | Match (_, _, _, matched_var, patterns) ->
      let pre_lambda_patterns =
        List.map patterns
          ~f:(convert_typed_ast_to_pre_lambda_pattern constructor_tag_map)
      in
  ...
  | BinaryOp (_, _, binary_op, expr_left, expr_right) ->
      let pre_lambda_expr_left =
        convert_typed_ast_to_pre_lambda constructor_tag_map expr_left
      in
      let pre_lambda_expr_right =
        convert_typed_ast_to_pre_lambda constructor_tag_map expr_right
      in
...
let convert_typed_ast_to_pre_lambda_function_defn
  ...
  let pre_lambda_function_body =
    convert_typed_ast_to_pre_lambda constructor_tag_map function_body
  in
...
let convert_typed_ast_to_pre_lambda_program
  ...
  let pre_lambda_function_defns =
    List.map typed_function_defn
      ~f:(convert_typed_ast_to_pre_lambda_function_defn constructor_tag_map)
  in
  let pre_lambda_main_expr_option =
    match typed_main_expr_option with
    | None -> None
    | Some main_expr ->
        Some (convert_typed_ast_to_pre_lambda constructor_tag_map main_expr)
  in
  TProg
