...
let rev_acc_lambda =
  ...
  let body =
    lfunction
      ...
      (Lifthenelse
          (Lvar xs,
          Llet (Strict, Pgenval, tl, (Lprim (Pfield (1, Pointer, Mutable), [Lvar xs], Loc_unknown)),
                Lsequence (Lprim (Psetfield (1, Pointer, Assignment), [Lvar xs; Lvar acc], Loc_unknown),
                            Lapply { ap_func = Lvar reverse_acc;
          ...
          Lvar acc))
  in
  Lprim (Psetglobal (Ident.create_persistent "Revacc"),
          [Lletrec ([reverse_acc, body],
                    Lprim (Pmakeblock (0, Mutable, Some [Pgenval]), [Lvar reverse_acc], Loc_unknown))],
          Loc_unknown)
...
let write_revacc () =
  ...
  Emitcode.to_file fd "Revacc" "revacc.cmo" ~required_globals:Ident.Set.empty instructions;
  close_out fd
