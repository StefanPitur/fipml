  ...
  | pattern_exprs ->
    ...
    List.iter
      (fun (MPattern (_, matched_expr, block_expr)) ->
        ...
        pprint_block_expr
          ppf
          ~indent:sub_expr_indent
          ~block_name:"PatternBlockExpr"
          block_expr)
      pattern_exprs
...
and pprint_matched_expr ppf ~indent matched_expr =
  ...
  | MConstructor (_, constructor_name, matched_exprs) ->
    print_matched_expr
      (Fmt.str "Constructor - %s" (Constructor_name.to_string constructor_name));
