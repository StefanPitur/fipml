...
let%expect_test "typing type defn: duplicated constructor definition in the same type" =
  let custom_type = 
    TType(mock_loc, Type_name.of_string "custom_type", [
      TTypeConstructor(mock_loc, Constructor_name.of_string "C1", []);
      TTypeConstructor(mock_loc, Constructor_name.of_string "C1", [TEUnit mock_loc])
    ])
  in
...
let%expect_test "typing type defn: duplicated constructor definition across different types" = 
  let custom_type_1 = 
    TType(mock_loc, Type_name.of_string "custom_type_1", [
      TTypeConstructor(mock_loc, Constructor_name.of_string "C1", [])
    ])
  in
  let custom_type_2 =
    TType(mock_loc, Type_name.of_string "custom_type_2", [
      TTypeConstructor(mock_loc, Constructor_name.of_string "C1", [TEUnit mock_loc])
    ])
  in
