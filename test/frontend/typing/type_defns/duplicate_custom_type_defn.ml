...
let%expect_test "typing type defn: duplicated custom type definition" = 
  ...
  let duplicated_custom_type =
    TType(mock_loc, Type_name.of_string "custom_type", [
      TTypeConstructor(mock_loc, Constructor_name.of_string "C2", [TEUnit mock_loc])
    ])
  in
  ...
  ;
  [%expect {| ("Typing.Type_defns_env.TypeAlreadyExists(\"File: mock - Line: 0 - Column: 1. Duplicate definition of type custom_type\")") |}]
